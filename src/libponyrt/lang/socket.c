  ...
  switch(iocp->op)
  {
    case IOCP_CONNECT:
    {
      if(err == ERROR_SUCCESS)
      {
        ...
        setsockopt((SOCKET)iocp->ev->asio_event_attr.fd, SOL_SOCKET,
    ...
    case IOCP_ACCEPT:
    {
      ...
      if(err == ERROR_SUCCESS)
      {
        ...
        SOCKET s = (SOCKET)iocp->ev->asio_event_attr.fd;
...
{
  SOCKET s = (SOCKET)ev->asio_event_attr.fd;
...
PONY_API int pony_os_accept(asio_event_t* ev)
{
  ...
  int ns = accept4(ev->asio_event_attr.fd, NULL, NULL, SOCK_NONBLOCK);

  if(ns == -1 && (errno == EWOULDBLOCK || errno == EAGAIN))
    ns = 0;
#else
  int ns = accept(ev->asio_event_attr.fd, NULL, NULL);
...
{
  ssize_t sent = writev(ev->asio_event_attr.fd, iov, iovcnt);
...
PONY_API size_t pony_os_send(asio_event_t* ev, const char* buf, size_t len)
{
  ...
  return 0;
#else
  ssize_t sent = send(ev->asio_event_attr.fd, buf, len, 0);
...
PONY_API size_t pony_os_recv(asio_event_t* ev, char* buf, size_t len)
{
  ...
  return 0;
#else
  ssize_t received = recv(ev->asio_event_attr.fd, buf, len, 0);
  ...
  ipaddress_t* ipaddr)
{
  ...
  ssize_t recvd = recvfrom(ev->asio_event_attr.fd, (char*)buf, len, 0,
